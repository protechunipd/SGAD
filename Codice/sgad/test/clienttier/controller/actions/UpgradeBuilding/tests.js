test("Test su UpgradeBuilding", function () {
    ...
    var datasTwo = {
        ...
        ownedUnits: [
            {unit: "Lavoratore", quantity: 8},
            {unit: "Cavaliere", quantity: 10},
            {unit: "Carro d'assalto", quantity: 2},
            {unit: "Fante", quantity: 5}
        ],
        ownedBuildings: [
            {building: "MinieraL1", position: {x: 4, y: 9}, isFinished: 1, time: 1, unitInProgress: {startedtime: 1, unit: "Cavaliere", quantity: 50}},
            {building: "MinieraL2", position: {x: 4, y: 9}, isFinished: 1, time: 1, unitInProgress: {startedtime: 1,  unit: "Cavaliere", quantity: 50}}
        ]
    }};
	...
	var buildingWithLevel = DataFactory.getInstance().getBuilding("MinieraL1");
	var buildingPossession = new BuildingPossession(buildingWithLevel, 1, new Position(0, 0), 0, 0, null);

    // Test su TrainUnit
    var upgradeBuilding = new UpgradeBuilding(buildingPossession);
	upgradeBuilding.setActionDatas(true);
    deepEqual(buildingPossession.getBuilding().getKey(), "MinieraL1", "Test su UpgradeBuilding");
	upgradeBuilding.performAction();
	deepEqual(buildingPossession.getBuilding().getKey(), "MinieraL2", "Test su UpgradeBuilding");
	upgradeBuilding.performAction();
	deepEqual(buildingPossession.getBuilding().getKey(), "MinieraL3", "Test su UpgradeBuilding");
	upgradeBuilding.setActionDatas(false);
	try {
		upgradeBuilding.performAction();
	} catch (e) {}
	deepEqual(buildingPossession.getBuilding().getKey(), "MinieraL3", "Test su UpgradeBuilding");
